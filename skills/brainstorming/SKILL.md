...
| :--- | :--- | :--- |
| **Discovery & Audit** | `prd-architect`, `app-analyzer-optimizer`, `web-scraper`, `website-design-cloner`, `browser-automation-expert`, `dependency-upgrade-migrator`, `project-context-mapper`, `proactive-background-watcher`, `self-evolving-memory-graph` | Formulate formal PRD, audit existing codebases, reverse-engineer website URL designs 1:1, scrape reference products, execute autonomous Playwright/Browser-Use web agents, or audit dependency health. |
...
| **AI & LLM Integration** | `ai-llm-integration-expert`, `vector-db-rag-expert`, `mcp-server-architect`, `multi-agent-orchestration`, `gemini-agent-booster`, `ai-cost-token-optimizer`, `autonomous-swarm-director`, `mcp-client-orchestrator` | RAG pipelines, MCP Server v1.9+, LangGraph multi-agent workflows, Gemini Agent long-context reasoning, LLM FinOps & prompt caching, autonomous agent swarms. |
...
| **Integrasi AI & LLM** | `ai-llm-integration-expert`, `vector-db-rag-expert`, `mcp-server-architect`, `multi-agent-orchestration`, `gemini-agent-booster`, `ai-cost-token-optimizer`, `autonomous-swarm-director`, `mcp-client-orchestrator` | Alur RAG, MCP Server v1.9+, workflow multi-agent LangGraph, OpenAI Agents SDK, Google ADK, pemikiran long-context Gemini, FinOps LLM & prompt caching. |
...
| **Penanganan Error & Ketahanan** | `error-resilience-expert`, `logging-error-tracking-expert` | React Error Boundaries, standar error API (RFC 9457), pola retry, circuit breaker, degradasi anggun, logging terstruktur (Pino), Sentry integrasi, korelasi request. |
