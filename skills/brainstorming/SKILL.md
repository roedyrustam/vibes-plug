...
| **AI & LLM Integration** | `ai-llm-integration-expert`, `mcp-server-architect`, `doku-mcp-server`, `multi-agent-orchestration`, `gemini-agent-booster` | RAG pipelines, Vercel AI SDK, MCP Server tool architectures (MCP v1.9+), DOKU Agentic Commerce MCP, stateful LangGraph multi-agent workflows, OpenAI Agents SDK, Google ADK, Gemini Agent long-context reasoning. |
| **SaaS & Multi-Tenancy** | `saas-mvp-launcher`, `saas-transformer`, `saas-multi-tenant`, `saas-billing`, `payment-gateway-expert`, `doku-payment-gateway` | SaaS MVP roadmap, RLS tenant isolation, Stripe/Polar.sh/DOKU billing, DOKU Jokul API integration, converting apps to multi-tenant SaaS platforms. **Always enforce Super Admin dashboard on a separate subdomain** (e.g., `admin.example.com`). |
