...
| :--- | :--- | :--- |
| **Discovery & Audit** | `prd-architect`, `app-analyzer-optimizer`, `web-scraper`, `website-design-cloner`, `browser-automation-expert`, `dependency-upgrade-migrator` | Formulate formal PRD, audit existing codebases, reverse-engineer website URL designs 1:1, scrape reference products, execute autonomous Playwright/Browser-Use web agents, or audit dependency health. |
| **UI/UX & Design Systems** | `design-system-architect`, `website-design-cloner`, `hig`, `monday-design-aesthetic`, `ui-ux-pro-max`, `ui-ux-expert`, `tailwind-expert`, `bootstrap-to-modern`, `ui-components-expert` | Reverse-engineer website designs & templates directly from target URL, design tokens (OKLCH), Tailwind CSS v4, Apple HIG principles, layout hierarchy, production-quality UI components, modernizing legacy Bootstrap apps. |
| **Frontend Frameworks** | `senior-frontend`, `tanstack-query-expert`, `global-a11y-i18n-expert`, `state-management-expert`, `form-validation-expert` | React 19 / Next.js 15 App Router (RSC, PPR), async state management (Zustand/Jotai), WCAG 2.2 a11y, multi-language i18n, complex forms with React Hook Form + Zod. |
...
| **Backend & Runtimes** | `js-backend-expert`, `python-programming-expert`, `go-programming-expert`, `rust-programming-expert`, `bun-runtime-expert`, `mvc-expert`, `fullstack-expert`, `senior-fullstack`, `async-queue-temporal-expert`, `cron-scheduler-expert` | API design, Node.js 24, Bun 1.2+, FastAPI, Go microservices, Rust Axum, durable workflows (Temporal/Trigger.dev/BullMQ), scheduled tasks & cron jobs. |
| **API Design & Contracts** | `api-design-expert`, `typescript-expert`, `rate-limit-abuse-prevention` | REST best practices, GraphQL schema, gRPC/protobuf, tRPC end-to-end type safety, OpenAPI 3.1 documentation, rate limiting, API key management, idempotency, versioning strategy. |
...
| **AI & LLM Integration** | `ai-llm-integration-expert`, `mcp-server-architect`, `doku-mcp-server`, `multi-agent-orchestration`, `gemini-agent-booster`, `ai-cost-token-optimizer` | RAG pipelines, MCP Server v1.9+, LangGraph multi-agent workflows, Gemini Agent long-context reasoning, LLM FinOps & prompt caching. |
...
| **Testing & Security** | `e2e-testing-expert`, `browser-automation-expert`, `zero-trust-secret-vault`, `vibe-code-gardener`, `coderabbit`, `authentication-identity-expert`, `rate-limit-abuse-prevention` | Playwright E2E suites, automated web agent flows, Zero-Trust secrets (Infisical/Vault), purging AI slop, automated code review, auth (Clerk/Supabase Auth/WebAuthn), brute-force & DDoS prevention. |
| **Error Handling & Resilience** | `error-resilience-expert`, `logging-error-tracking-expert` | React Error Boundaries, API error standards (RFC 9457), retry patterns, circuit breakers, graceful degradation, structured logging (Pino), Sentry integration, request correlation. |
...
| **DevOps & CI/CD** | `ci-cd-devops-architect`, `cloud-hosting-expert`, `dependency-upgrade-migrator` | GitHub Actions, Docker, Kubernetes, Terraform, Vercel/Cloudflare deployment, Renovate Bot, dependency health. |
...
| :--- | :--- | :--- |
| **Penemuan & Audit** | `prd-architect`, `app-analyzer-optimizer`, `web-scraper`, `website-design-cloner`, `browser-automation-expert`, `dependency-upgrade-migrator` | Menyusun PRD formal, mengaudit codebase yang ada, mendokumentasikan fitur kompetitor, reverse-engineer desain website, atau audit kesehatan dependensi. |
| **UI/UX & Design System** | `design-system-architect`, `website-design-cloner`, `hig`, `monday-design-aesthetic`, `ui-ux-pro-max`, `ui-ux-expert`, `tailwind-expert`, `bootstrap-to-modern`, `ui-components-expert` | Token desain (OKLCH), Tailwind CSS v4, prinsip Apple HIG, tata letak antarmuka, komponen UI berkualitas produksi, migrasi aplikasi Bootstrap lama. |
| **Framework Frontend** | `senior-frontend`, `tanstack-query-expert`, `global-a11y-i18n-expert`, `state-management-expert`, `form-validation-expert` | React 19 / Next.js 15 App Router (RSC, PPR), state management (Zustand/Jotai), WCAG 2.2 a11y, i18n multi-bahasa, formulir kompleks dengan React Hook Form + Zod. |
| **Mobile & Desktop** | `mobile-expo-expert`, `tauri-expert`, `mobile-push-notification-expert` | Aplikasi mobile cross-platform React Native / Expo Router v4, iOS Live Activities, push notification (FCM/APNs), atau aplikasi desktop Tauri v2 berbasis Rust. |
| **Backend & Runtime** | `js-backend-expert`, `python-programming-expert`, `go-programming-expert`, `rust-programming-expert`, `bun-runtime-expert`, `mvc-expert`, `fullstack-expert`, `senior-fullstack`, `async-queue-temporal-expert`, `cron-scheduler-expert` | Desain API, Node.js 24, Bun 1.2+, FastAPI 0.115+, microservices Go 1.24+, Rust Axum, refactoring MVC PHP lama, tugas terjadwal & cron job. |
| **Desain API & Kontrak** | `api-design-expert`, `typescript-expert`, `rate-limit-abuse-prevention` | Praktik terbaik REST, skema GraphQL, gRPC/protobuf, tRPC end-to-end type safety, dokumentasi OpenAPI 3.1, rate limiting, manajemen API key, idempotency, strategi versioning. |
...
| **Integrasi AI & LLM** | `ai-llm-integration-expert`, `mcp-server-architect`, `doku-mcp-server`, `multi-agent-orchestration`, `gemini-agent-booster`, `ai-cost-token-optimizer` | Alur RAG, MCP Server v1.9+, workflow multi-agent LangGraph, OpenAI Agents SDK, Google ADK, pemikiran long-context Gemini, FinOps LLM & prompt caching. |
...
| **Testing & Keamanan** | `e2e-testing-expert`, `browser-automation-expert`, `zero-trust-secret-vault`, `vibe-code-gardener`, `coderabbit`, `authentication-identity-expert`, `rate-limit-abuse-prevention` | Testing E2E Playwright, automated web agent, rahasia Zero-Trust (Infisical/Vault), pembersihan AI slop, code review otomatis, auth (Clerk/Supabase Auth/WebAuthn), pencegahan brute-force & DDoS. |
| **Penanganan Error & Ketahanan** | `error-resilience-expert`, `logging-error-tracking-expert` | React Error Boundaries, standar error API (RFC 9457), pola retry, circuit breaker, degradasi anggun, logging terstruktur (Pino), integrasi Sentry, korelasi request. |
...
| **DevOps & CI/CD** | `ci-cd-devops-architect`, `cloud-hosting-expert`, `dependency-upgrade-migrator` | GitHub Actions, Docker, Kubernetes, Terraform, deployment Vercel/Cloudflare, Renovate Bot, kesehatan dependensi. |
